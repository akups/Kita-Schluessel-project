...
router.get("/discover/hamburg/:id", (req, res) => {
  HamburgKita.findById(req.params.id).then(kita => {
    res.json(kita);
  });
});

//post new kita in Hamburg
router.post("/addkita/hamburg", (req, res) => {
  //2. Get the data from frontend
  const {
    name,
    district,
...
//delete Kita in Hamburg
router.delete("/discover/hamburg/:id/delete", (req, res) => {
  console.log("whats req hamb?", req.params.id);
  HamburgKita.deleteOne({ _id: req.params.id })
    .then(result => {
      //console.log("RESULT", result);
      res.json(result);
    })
